...
import { createLeadFromInsight, runCompetitorSpy, type CompetitorSpyState } from "@/app/actions";
...
export function CompetitorSpyForm() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <form action={action} className="mt-6 space-y-4 rounded-2xl border border-[#e3dccd] bg-white p-4">
              ...
              >
                {pending ? "Submitting..." : "Generate competitor brief"}
              </button>
      ...
      {job ? <ToolJobStatus job={job} title="Competitor Spy job" /> : null}
      ...
      {result ? (
        <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="space-y-6">
            ...
            <Panel title="Turn this into a workspace asset" eyebrow="Use the intel inside LeadForge">
              <div className="grid gap-3">
                <form action={createLeadFromInsight} className="rounded-xl border border-[#e3dccd] bg-white p-4">
                  ...
                  <input type="hidden" name="segment" value="Competitor intelligence" />
                  <input type="hidden" name="source" value="competitor_spy" />
                  <input type="hidden" name="tags" value="competitor, strategy, growth-lab" />
                  <input
                    type="hidden"
                    name="notes"
                    value={`Summary: ${result.summary}\nAttack plan: ${result.quickAttackPlan.homepageAngle} | ${result.quickAttackPlan.proofStrategy} | ${result.quickAttackPlan.ctaStrategy}`}
                  />
                  <p className="text-sm font-black text-[#1e2521]">Save intelligence target</p>
                  <p className="mt-2 text-sm leading-6 text-[#4f5a53]">Store this market context in the pipeline so the team can reference it while shaping messaging and approvals.</p>
