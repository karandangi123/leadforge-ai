...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile Bottom Dock */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around h-20 px-4 pb-4 border-t border-gray-100 bg-white/80 backdrop-blur-xl">
        {navItems.map((item) => (
          <a
            key={item.label}
            href={item.href}
            className="flex flex-col items-center gap-1.5 p-2 text-gray-400 hover:text-emerald-600 transition-colors"
          >
            <item.icon size={20} />
