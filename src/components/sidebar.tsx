    ...
    <>
      ...
      <header className="lg:hidden sticky top-0 z-50 flex items-center justify-between h-16 px-4 border-b border-gray-100 bg-white/80 backdrop-blur-xl">
        <button onClick={toggleMenu} className="flex items-center gap-2 group">
          <div className={bt + "size-10 rounded-xl bg-gray-900 flex items-center justify-center shadow-lg transition-transform duration-300 " + bt + "{isMenuOpen ? 'rotate-180' : ''}" + bt + bt}>
      ...
      {/* Sidebar Content */}
      <div className={bt + "fixed lg:static inset-y-0 left-0 z-40 w-72 bg-white border-r border-gray-100 transform " + bt + "{isMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col" + bt}>
            ...
            <div className="space-y-1">
              {navItems.map((item) => (
                <Link
                  ...
                  className={bt + "flex items-center gap-4 px-4 py-3 rounded-2xl transition-all duration-200 " + bt + "{pathname === item.href ? 'bg-gray-900 text-white shadow-lg shadow-gray-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}" + bt}
                >
